...
$M = new CountyClass;
	...
	{
		$county_id = $_REQUEST['delid'];
		
		/* delete Record - call function delete -  returns integer
		 * @param string $fieldName,  @param array $matchingValue
		 */
	
		$RecoredDeleted = $M->delete('county_id',$county_id);
		
		if ($RecoredDeleted == 1)
			header("Location: county.php?msg=dl");
		if ($RecoredDeleted == 2)
			header("Location: county.php?msg=errd");
	...
	if (isset($_REQUEST['action']) && ($_REQUEST['action']== "delsel"))
	{
		 ...
		 */
		  if (count($ArrayId) > 0) { 
			$RecoredDeleted = $M->deleteArray('county_id',$ArrayId);
			
			if ($RecoredDeleted == 1)
				header("Location: county.php?msg=dlsl");
			else 
				header("Location: county.php?msg=err");
		  }
		  else {
...
function deleteItem(item_id,item_name)
{
	...
	{
		document.location.href="county.php?m=del&delid="+item_id;
	}
}
...
<div class="maincontent1">
  <div class="breadcrumbs"> <a href="index.php">Dashboard</a> <span>County</span> </div>
  <!-- breadcrumbs -->
  
  <div class="left">
    <h1 class="pageTitle">Manage County </h1>
    <a href="newcounty.php" class="addNewButton">Add New County </a>
    ...
    <div class="sTableWrapper">
      <table cellpadding="0" cellspacing="0" class="sTable" id="userlist" width="100%">
        ...
        <?php
					...
					{
								$county_id = $recordList[$i]['county_id'];
								$county = $recordList[$i]["county"];
						
				?>
        <tr>
          <td align="center" width="2%"><?php if ($userType == 'Admin') { ?>
              <input name="uid[]" type="checkbox" id="<?php echo $county_id?>" value="<?php echo $county_id?>" />
            <?php  } ?></td>
          <td width="88%"><?php echo $county; ?></td>
          <td width="10%" align="center"><?php if ($userType == 'Admin') { ?>
              <a href="newcounty.php?fl=edit&amp;id=<?php echo $county_id;?>"><img style="vertical-align:middle" src="images/icons/small/black/edit.png" alt="Edit" width="15" height="16" border="0" /></a> &nbsp; <a href="javascript:deleteItem('<?php echo $county_id;?>','<?php echo $county;?>');" class=""><img style="vertical-align:middle" src="images/icons/small/black/close.png" alt="Delete" width="16" height="16" border="0" /></a>
              <?php } ?>          </td>
        </tr>
        <?php 
					} 
