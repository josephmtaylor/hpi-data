...
$DA = new CenterAddressClass;
	...
	if (isset($_GET['m']) && ($_GET['m']== "del"))
	{
		...
		$RecoredDeleted = $M->delete('center_id',$id);
		
		if ($RecoredDeleted == 1)
			header("Location: center.php?msg=dl".$page);
		if ($RecoredDeleted == 2)
			header("Location: center.php?msg=errd".$page);
		else  if ($RecoredDeleted == 0)
			header("Location: center.php?msg=err".$page);
	...
	if (isset($_POST['action']) && ($_POST['action']== "cndeleted"))
	{
		...
		$RecoredDeleted =  $M->deleteArray('center_id',$ArrayId);
		
		if ($RecoredDeleted == 1)
			header("Location: center.php?msg=dlsl".$page);
		else 
			header("Location: center.php?msg=err".$page);
	...
	if (isset($_POST['action']) && ($_POST['action']== "cnverified" or $_POST['action']== "cnunverified" or $_POST['action']== "cnpending" or $_POST['action']== "cndone"))
	{
		...
		if ($action == 'cnverified')
			...
			$status = 'UV';	
		else if ($action == 'cnpending')
			$status = 'P';
		else if ($action == 'cndone')
			$status = 'D';
		
	
		$updateStatus = $M->changeCenterRecordArray($ArrayId, $status);
			
		if ($updateStatus == 1)
			header("Location: center.php?msg=cst". $suffix . $page);
		else 
			header("Location: center.php?msg=err". $suffix . $page);
	...
	if (isset($_REQUEST['mode']) && ($_REQUEST['mode']== "search")) {
			...
			$searchField= array($key1); 
			$searchKeyword = array( $M->prepare_input($searchkey) );
			$searchStatus = '';
...
function deleteItem(item_id,item_name, page)
{
	...
	{
		document.location.href="center.php?m=del&delid="+item_id+"&page="+page;
	}
}
...
<div class="maincontent1">
  <div class="breadcrumbs"> <a href="index.php">Dashboard</a> <span>Centers</span> </div>
  <!-- breadcrumbs -->
  
  <div class="left">
    <h1 class="pageTitle">Manage Centers </h1>
    <a href="newcenter.php?fl=new<?php echo $suffix; ?>" class="addNewButton">Add New Center </a>
	...
	<ul class="submenu">
		...
		<li> <form id="fm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post"> 
				<input name="searchkey" type="text" id="searchkey" value="">
				<select name="key1">
					<option value="name">By Name</option><option value="phone">By Phone</option><option value="npi">By NPI</option>
				</select>
				<input name="mode" type="hidden" value="search" /><button>Search</button></form></li>
    </ul>
      ...
      <table cellpadding="0" cellspacing="0" class="sTable" id="userlist" width="100%">
        ...
        <?php
					for($i=0; $i < $totalLimit ; $i++)
					{
								...
								$chapter_name = 'N/A';  $category_name = 'N/A';
								if (!empty($chapter_id_fk)) {
									$lrec = $CP->getRecordByField('chapter_id', $chapter_id_fk,'exact');
									$chapter_name = $lrec['chapter_name'];
								}
								if (!empty($category_id_fk)) {
									$srec = $CT->getRecordByField('cat_id', $category_id_fk,'exact');
									$category_name = $srec['category_name'];
								}
								...
								$daRec = $DA->getCenterAddressRecord($fieldName='center_id', $id);
        ...
        <tr>
		  ...
		  <td align="center">
            ...
            <a title="Edit Center Information" href="newcenter.php?fl=edit&amp;id=<?php echo $id . $suffix;?>"><img style="vertical-align:middle" src="images/icons/small/black/edit.png" alt="Edit" width="16" height="16" border="0" /></a>&nbsp;
			<a title="View Center Info" href="centerpublication.php?id=<?php echo $id;?>"><img style="vertical-align:middle" src="images/icons/small/black/search.png" alt="View" width="16" height="16" border="0" /></a>
           </td>
        </tr>
        <?php 
					} 
