...
$M = new CategoryClass;
	...
	if (isset($_REQUEST['m']) && ($_REQUEST['m']== "del"))
	{
		...
		$RecoredDeleted = $M->delete('cat_id',$cat_id);
		
		if ($RecoredDeleted == 1)
			header("Location: category.php?msg=dl");
		if ($RecoredDeleted == 2)
			header("Location: category.php?msg=errd");
	...
	if (isset($_REQUEST['action']) && ($_REQUEST['action']== "delsel"))
	{
		 ...
		 */
		  if (count($ArrayId) > 0) { 
			$RecoredDeleted = $M->deleteArray('cat_id',$ArrayId);
			
			if ($RecoredDeleted == 1)
				header("Location: category.php?msg=dlsl");
			else 
				header("Location: category.php?msg=err");
		  }
		  else {
...
function deleteItem(item_id,item_name)
{
	...
	{
		document.location.href="category.php?m=del&delid="+item_id;
	}
}
...
<div class="maincontent1">
  <div class="breadcrumbs"> <a href="index.php">Dashboard</a> <span>Category</span> </div>
  <!-- breadcrumbs -->
  
  <div class="left">
    <h1 class="pageTitle">Manage Category </h1>
    <a href="newcategory.php" class="addNewButton">Add New Category </a>
    ...
    <div class="sTableWrapper">
      <table cellpadding="0" cellspacing="0" class="sTable" id="userlist" width="100%">
        ...
        <?php
					...
					{
								$cat_id = $recordList[$i]['cat_id'];
								$category_name = $recordList[$i]["category_name"];
						
				?>
        <tr>
          <td align="center" width="2%"><?php if ($userType == 'Admin') { ?>
              <input name="uid[]" type="checkbox" id="<?php echo $cat_id?>" value="<?php echo $cat_id; ?>" />
            <?php  } ?></td>
          <td width="88%"><?php echo $category_name; ?></td>
          <td width="10%" align="center"><?php if ($userType == 'Admin') { ?>
              <a href="newcategory.php?fl=edit&amp;id=<?php echo $cat_id;?>"><img style="vertical-align:middle" src="images/icons/small/black/edit.png" alt="Edit" width="15" height="16" border="0" /></a> &nbsp; <a href="javascript:deleteItem('<?php echo $cat_id;?>','<?php echo $category_name;?>');" class=""><img style="vertical-align:middle" src="images/icons/small/black/close.png" alt="Delete" width="16" height="16" border="0" /></a>
              <?php } ?>          </td>
        </tr>
        <?php 
					} 
