...
$M = new CityClass;
	...
	{
		$city_id = $_REQUEST['delid'];
		
		/* delete Record - call function delete -  returns integer
		 * @param string $fieldName,  @param array $matchingValue
		 */
	
		$RecoredDeleted = $M->delete('city_id',$city_id);
		
		if ($RecoredDeleted == 1)
			header("Location: city.php?msg=dl");
		if ($RecoredDeleted == 2)
			header("Location: city.php?msg=errd");
	...
	if (isset($_REQUEST['action']) && ($_REQUEST['action']== "delsel"))
	{
		 ...
		 */
		  if (count($ArrayId) > 0) { 
			$RecoredDeleted = $M->deleteArray('city_id',$ArrayId);
			
			if ($RecoredDeleted == 1)
				header("Location: city.php?msg=dlsl");
			else 
				header("Location: city.php?msg=err");
		  }
		  else {
...
function deleteItem(item_id,item_name)
{
	...
	{
		document.location.href="city.php?m=del&delid="+item_id;
	}
}
...
<div class="maincontent1">
  <div class="breadcrumbs"> <a href="index.php">Dashboard</a> <span>City</span> </div>
  <!-- breadcrumbs -->
  
  <div class="left">
    <h1 class="pageTitle">Manage City </h1>
    <a href="newcity.php" class="addNewButton">Add New City </a>
    ...
    <div class="sTableWrapper">
      <table cellpadding="0" cellspacing="0" class="sTable" id="userlist" width="100%">
        ...
        <?php
					...
					{
								$city_id = $recordList[$i]['city_id'];
								$city = $recordList[$i]["city"];
						
				?>
        <tr>
          <td align="center" width="2%"><?php if ($userType == 'Admin') { ?>
              <input name="uid[]" type="checkbox" id="<?php echo $city_id?>" value="<?php echo $city_id?>" />
            <?php  } ?></td>
          <td width="88%"><?php echo $city; ?></td>
          <td width="10%" align="center"><?php if ($userType == 'Admin') { ?>
              <a href="newcity.php?fl=edit&amp;id=<?php echo $city_id;?>"><img style="vertical-align:middle" src="images/icons/small/black/edit.png" alt="Edit" width="15" height="16" border="0" /></a> &nbsp; <a href="javascript:deleteItem('<?php echo $city_id;?>','<?php echo $city;?>');" class=""><img style="vertical-align:middle" src="images/icons/small/black/close.png" alt="Delete" width="16" height="16" border="0" /></a>
              <?php } ?>          </td>
        </tr>
        <?php 
					} 
