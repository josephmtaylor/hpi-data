...
$M = new SpecialtyClass;
	...
	{
		$specialty_id = $_REQUEST['delid'];
		
		/* delete Record - call function delete -  returns integer
		 * @param string $fieldName,  @param array $matchingValue
		 */
	
		$RecoredDeleted = $M->delete('specialty_id',$specialty_id);
		
		if ($RecoredDeleted == 1)
			header("Location: specialty.php?msg=dl");
		if ($RecoredDeleted == 2)
			header("Location: specialty.php?msg=errd");
	...
	if (isset($_REQUEST['action']) && ($_REQUEST['action']== "delsel"))
	{
		 ...
		 */
		  if (count($ArrayId) > 0) { 
			$RecoredDeleted = $M->deleteArray('specialty_id',$ArrayId);
			
			if ($RecoredDeleted == 1)
				header("Location: specialty.php?msg=dlsl");
			else 
				header("Location: specialty.php?msg=err");
		  }
		  else {
...
function deleteItem(item_id,item_name)
{
	...
	{
		document.location.href="specialty.php?m=del&delid="+item_id;
	}
}
...
<div class="maincontent1">
  <div class="breadcrumbs"> <a href="index.php">Dashboard</a> <span>Specialty</span> </div>
  <!-- breadcrumbs -->
  
  <div class="left">
    <h1 class="pageTitle">Manage Specialty </h1>
    <a href="newspecialty.php" class="addNewButton">Add New Specialty </a>
    ...
    <div class="sTableWrapper">
      <table cellpadding="0" cellspacing="0" class="sTable" id="userlist" width="100%">
        ...
        <?php
					...
					{
								$specialty_id = $recordList[$i]['specialty_id'];
								$specialty_name = $recordList[$i]["specialty_name"];
						
				?>
        <tr>
          <td align="center" width="2%"><?php if ($userType == 'Admin') { ?>
              <input name="uid[]" type="checkbox" id="<?php echo $specialty_id?>" value="<?php echo $specialty_id?>" />
            <?php  } ?></td>
          <td width="88%"><?php echo $specialty_name; ?></td>
          <td width="10%" align="center"><?php if ($userType == 'Admin') { ?>
              <a href="newspecialty.php?fl=edit&amp;id=<?php echo $specialty_id;?>"><img style="vertical-align:middle" src="images/icons/small/black/edit.png" alt="Edit" width="15" height="16" border="0" /></a> &nbsp; <a href="javascript:deleteItem('<?php echo $specialty_id;?>','<?php echo $specialty_name;?>');" class=""><img style="vertical-align:middle" src="images/icons/small/black/close.png" alt="Delete" width="16" height="16" border="0" /></a>
              <?php } ?>          </td>
        </tr>
        <?php 
					} 
