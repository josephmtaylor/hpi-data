<?php
/**
 * PHP Class to publication access (dbquery, dbfetch , etc)
 * 
 * <code><?php
 * include('publication.class.php');
 * $Ad = new PublicationClass();
 * ?></code>
 * 
 * ==============================================================================
 * 
 * @version $Id: publication.class.php,v 0.01 2012/02/12 12:12:32 $
...
class PublicationClass extends Db { 
		...
		{ 	
			$selectedFields = "publication";
			$fieldName = 'pub_id';
			$searchValue = $id;
			$dbrow = $this->getSelectedFieldsRecordByField($selectedFields, $fieldName, $searchValue);
			return $dbrow['publication'];

		}
		
		function editPublication($sessionValues)
		{
				//print_r($sessionValues);
				$fl 		  = 	$sessionValues['fl'];
				$id 		  = 	$sessionValues['pub_id'];
				$publication  =		$this->prepare_input($sessionValues['publication']);
				$description  =		$this->prepare_input($sessionValues['description']);
			
				$recordCreated = $this->isIdPresent($id);
				
				if ($recordCreated == 0) 
				{
						$sqlQuery = "INSERT INTO  ". $this->table ."  ( `pub_id` , `publication` , `description`  )  VALUES ( NULL, '$publication' , '$description' ) ";
				...
				{
						$sqlQuery = "UPDATE ". $this->table ." SET `publication`  = '$publication' , `description` = '$description'  WHERE pub_id = $id ";
						$sqlQuery = $this->dbquery($sqlQuery) or die(mysql_error());	
