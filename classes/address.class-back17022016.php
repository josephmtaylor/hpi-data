...
Original Phone Text: ' . $phone;
				...
				if ($isRecordFound == 0) 
				{
						$sqlQuery = "INSERT INTO  ". $this->table ."  ( `id` , `address_1` , `address_2` , `city_id` , `state_id` , `zip_id`, `county_id` , `phone` , `fax` , `notes` ,`status` , `date_created` , `date_updated` , `updated_by`)  
									 VALUES ( NULL, '$address_1' , '$address_2' , '$city_id' , '$state_id' , '$zip_id', '$county_id' , '$phone' , '$fax' , '$notes' , '$status' , NOW() , NOW() , '$updated_by' ) ";
				...
				{
						$sqlQuery = "UPDATE ". $this->table ." SET `address_1`  = '$address_1' , `address_2` = '$address_2' , `city_id` = '$city_id' , `state_id` = '$state_id' , `zip_id` = '$zip_id' , `county_id` = '$county_id' , `phone` = '$phone' , `fax` = '$fax' , `notes` = '$notes' , `status` = '$status' , `date_updated` = NOW(), `updated_by` = '$updated_by'  WHERE id = $id ";
		...
		function getTotalSearchArray($fieldNameArray='', $searchValueArray='', $searchStatus='', $orderByField='', $orderByValue='', $offset='', $limit='')
		{
			...
			$sqlQuery = "SELECT A.id, A.address_1, A.address_2, A.phone, A.fax, A.status,  A.date_updated,  A.updated_by, C.city, S.state, Z.zipcode, D.doctor_id, 
								D.first_name, D.middle_name, D.last_name,D.fullname, D.email, D.website, D.status as dstatus, D.npi, D.specialty_id_fk, DS.specialty_name ";
		...
		function getTotalCenterSearchArray($fieldNameArray='', $searchValueArray='', $searchStatus='', $orderByField='', $orderByValue='', $offset='', $limit='')
		{
			...
			$sqlQuery = "SELECT DISTINCT(A.id), A.address_1, A.address_2, A.city_id, A.state_id, A.zip_id, A.phone, A.fax, A.status, A.date_updated, A.updated_by, C.center_id, 
								C.name, C.status as dstatus,  C.category_id_fk, DC.category_name ";
