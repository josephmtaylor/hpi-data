...
/*		function checkEmailForUpdate($id, $email) 
		...
		function createCenter($sessionValues)
		{
				...
				if ($import == 1) { 
					...
					$keyArray = array($this->prepare_input($name),  $this->prepare_input($category_id_fk));
					$CenterCreated = $this->getTotalCenterBySearchArray($fieldArray, $keyArray, $searchStatus='exact');
					
					$docRec = $this->getSelectedFieldsRecordListArray($selectedFields='center_id', $fieldArray, $keyArray, $searchStatus='exact');
					$centerid = $docRec[0]['center_id'];
				}

				if ($CenterCreated == 0 ) 
				{
						$sqlQuery = "INSERT INTO ". $this->table ." ( `center_id` , `name` , `specialty_id_fk`,  `category_id_fk`,  `chapter_id_fk` , `description`, `notes` ,  `website`, `npi` , `admission` ,  `status` , `date_created` , `date_updated` , `updated_by`  )  
									 VALUES (  NULL , '$name' , '$specialty_id_fk' , '$category_id_fk' , '$chapter_id_fk' , '$description' , '$notes' ,  '$website' , '$npi' , '$admission' , '$status' , NOW() , NOW() , '$updated_by' ) ";
				...
				if (!empty($address_1) or !empty($phone)) { 
					$A = new AddressClass;
					
					/*$fieldArray = array ('address_1', 'city_id', 'state_id' , 'zip_id', 'county_id', 'phone' , 'fax');
					$keyArray = array($this->prepare_input($address_1), $this->prepare_input($city_id), $this->prepare_input($state_id), $this->prepare_input($zip_id) , $this->prepare_input($county_id_fk) , $this->prepare_input($phone) , $this->prepare_input($fax));*/
					//$county_id_fk = 0;
					
					$isAddressFound = 0; $notes = ''; $status = 'N';
					if (!empty($address_1)) { 
						$fieldArray = array ('address_1' );
						$keyArray = array($this->prepare_input($address_1));
						$isAddressFound = $A->getRecordCountBySearchArray($fieldArray, $keyArray, $searchStatus='exact');
					} else if (!empty($phone)) { 
						$fieldArray = array ('phone' );
						$keyArray = array($this->prepare_input($phone));
						$isAddressFound = $A->getRecordCountBySearchArray($fieldArray, $keyArray, $searchStatus='exact');
					}
				
					if ($isAddressFound == 0) {
							$arrayValues = array (  "fl" => 'add', "id" => 0, "center_id" => $centerid, "address_1" => $address_1, "address_2" => $address_2, 
													'city_id' => $city_id, 'state_id' => $state_id, 'zip_id' => $zip_id, 'county_id' => $county_id_fk , 'phone' => $phone, 
													'fax' => $fax , 'notes' => $notes, 'status' => $status, 'updated_by' => $updated_by
												  );
		...
		function editCenter($sessionValues)
		{
				...
				if ($CenterCreated == 0 ) 
				{
						$sqlQuery = "INSERT INTO ". $this->table ." ( `center_id` , `name` , `description`,  `notes` , `specialty_id_fk`,  `category_id_fk` ,   `npi` ,  `status` , `date_created` , `date_updated` , `updated_by`  )  
									 VALUES (  NULL , '$name' , '$description' , '$notes' , '$specialty_id_fk' , '$category_id_fk' ,  '$npi' ,  '$status' , NOW() , NOW() , '$updated_by' ) ";
						$sqlQueryResult = $this->dbquery($sqlQuery) or die(mysql_error());
