<?php
/**
 * PHP Class to doctor publication access (dbquery, dbfetch , etc)
 * 
 * <code><?php
 * include('doctorpublication.class.php');
 * $Ad = new DoctorPublicationClass();
 * ?></code>
 * 
 * ==============================================================================
 * 
 * @version $Id: doctorpublication.class.php,v 0.01 
...
class DoctorPublicationClass extends Db { 

		private $table = 'doctor_publication';
		private $data = array();
		...
		function deleteRecord($doctor_id, $pub_id, $year='')
		{
			
			$sqlQuery = "DELETE FROM `" . $this->table . "` WHERE  doctor_id = '$doctor_id' AND pub_id = '$pub_id' ";
		...
		function getDoctorPublicationRecord($fieldName, $searchKey, $searchStatus='',  $orderByField='', $orderByValue='', $offset='', $limit='')
		{
				$sqlQuery  =   "SELECT  DP.doctor_id, DP.pub_id, DP.year, P.publication
								FROM `doctor_publication` AS DP INNER JOIN `publications` AS P ON P.pub_id = DP.pub_id
		...
		function editDoctorPublication($sessionValues)
		{
				$fl 		= 	$sessionValues['fl'];
				...
				if ($isRecordExists == 0) 
				{
						$sqlQuery = "INSERT INTO ". $this->table ."  ( `doctor_id` , `pub_id` ,  `year` )  VALUES  ( '$doctor_id', '$pub_id', '$year'  ) ";
