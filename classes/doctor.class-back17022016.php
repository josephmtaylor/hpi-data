...
class DoctorClass extends Db { 
		...
		function createDoctor($sessionValues)
		{
				...
				if ($import == 1) { 
					...
					$keyArray = array($this->prepare_input($fullname), $this->prepare_input($npi),  $this->prepare_input($specialty_id_fk));
				...
				if ($DoctorCreated == 0 ) 
				{
						$sqlQuery = "INSERT INTO ". $this->table ." ( `doctor_id` , `first_name` , `middle_name`,  `last_name` , `fullname`,  `notes` , `specialty_id_fk`,  `category_id_fk` ,  `email`  ,  `license_status`, `website`,  `npi` , `image` ,  `status` , `date_created` , `date_updated` , `updated_by`  )  
									 VALUES (  NULL , '$first_name' , '$middle_name' , '$last_name' , '$fullname' , '$notes' , '$specialty_id_fk' , '$category_id_fk' , '$email' , '$license_status' , '$website' , '$npi' , '$image' , '$status' , NOW() , NOW() , '$updated_by' ) ";
				...
				if (!empty($address_1) and !empty($city_id) and !empty($state_id) and !empty($zip_id)) { 
					...
					if ($isAddressFound == 0) {
							$arrayValues = array (    	"fl" => 'add', "id" => 0, "doctor_id" => $doctorid,
														"address_1" => $address_1, "address_2" => $address_2, 'city_id' => $city_id, 'state_id' => $state_id, 'zip_id' => $zip_id,
														'county_id' => $county_id_fk , 'phone' => $phone, 'fax' => $fax, 'updated_by' => $loggedUser
												  );
