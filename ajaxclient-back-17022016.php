					...
					!empty($_POST['category_id_fk']) and !empty($_POST['npi']) and !empty($_POST['status']) ) { 
						...
						if ($doctorid == '' or $doctorid == 0) { 
								...
								$arrayValues = array (  "fl" => 'add', "doctor_id" => $doctorid, "full_name" => $full_name , "first_name" => $first_name, 
														"last_name" => $last_name, 'specialty_id_fk' => $specialty_id_fk, 'category_id_fk' => $category_id_fk, 'npi' => $npi,
						...
						if ($createDoctorAddress == 1) {
							...
							$jstr = array( 'success' => 1 , 'failure' => 0 , 'da_id' =>  $da_id , 'doctor_id' =>  $doctorid , 'doctor_name' =>  $doctor_name  , 'specialty' =>  $specialty  , 'npi' =>  $npi , 'notes' =>  $notes , 'current_status' =>  $current_status , 'new' => $new , 'message' => $msg); 
		...
		if ($action == 'getdoctorinfo') { 
				...
				if (!empty($_POST['doctor_id'])) { 
						...
						if ($isDoctorFound > 0 ) {
								...
								$jstr = array( 'success' => 1 , 'failure' => 0 , 'doctor_id' =>  $did , 'fullname' =>  $fullname , 'first_name' =>  $first_name , 'last_name' =>  $last_name  , 'npi' =>  $npi , 'specialty_id_fk' =>  $specialty_id_fk , 'category_id_fk' =>  $category_id_fk , 'notes' =>  $notes , 'status' =>  $status , 'message' => 'doctor found.'); 
				...
				if (!empty($_POST['adrid']) and  !empty($_POST['name'])  and  !empty($_POST['category_id_fk']) and  !empty($_POST['status']) ) { 
						...
						if ($centerid == '' or $centerid == 0) { 
								...
								$arrayValues = array (  "fl" => 'add', "center_id" => $centerid, "name" => $name, 'specialty_id_fk' => $specialty_id_fk, 
														'category_id_fk' => $category_id_fk, 'npi' => $npi, 'description' => $description, 'notes' => $notes, 'status' => $status
													  );
						...
						if ($createCenterAddress == 1) {
							...
							$jstr = array( 'success' => 1 , 'failure' => 0 , 'da_id' =>  $da_id , 'center_id' =>  $centerid , 'center_name' =>  $center_name  , 'category' =>  $category  , 'npi' =>  $npi , 'description' =>  $description , 'notes' =>  $notes , 'current_status' =>  $current_status , 'new' => $new , 'message' => $msg); 
		...
		if ($action == 'getcenterinfo') { 
				...
				if (!empty($_POST['center_id'])) { 
						...
						if ($isCenterFound > 0 ) {
								...
								$jstr = array( 'success' => 1 , 'failure' => 0 , 'center_id' =>  $did , 'name' =>  $name , 'npi' =>  $npi , 'specialty_id_fk' =>  $specialty_id_fk , 'category_id_fk' =>  $category_id_fk , 'notes' =>  $notes , 'description' => $description , 'status' =>  $status , 'message' => 'center found.'); 
